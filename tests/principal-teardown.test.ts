...
const seedActiveCustomer = async (
  subscriber: string,
  customerId: string,
...
describe('principal teardown — full purge of a solo principal', () => {
  it('deletes the identity + billing customer + group-of-one so nothing survives', async () => {
    ...
    expect(
      (await getBackends()).billing &&
        (await (await getBackends()).billing.read(APP_ID)).subscriptions[user.id],
    ).toBeTruthy();
    expect((await (await getBackends()).membership.read(APP_ID)).groups[grp]).toBeTruthy();

    // 1) Identity delete.
    const idRes = await server.inject({
      method: 'DELETE',
      url: `/auth/admin/identity/${user.id}`,
      headers: SVC,
    });
    ...
    const billRes = await server.inject({
      method: 'DELETE',
      url: '/billing/customer',
      headers: SVC,
      payload: { subscriber: user.id },
    });
    expect(billRes.statusCode).toBe(200);
    expect(billRes.json()).toMatchObject({
      deleted: true,
    ...
    const memRes = await server.inject({
      method: 'DELETE',
      url: `/identities/${user.id}/memberships`,
      headers: SVC,
    });
...
describe('principal teardown — shared group keeps the group, drops just the member', () => {
  it('removes only the departing member and emits membership.removed (via:teardown)', async () => {
    ...
    const res = await server.inject({
      method: 'DELETE',
      url: '/identities/memberB/memberships',
      headers: SVC,
    });
    ...
    const removed = evs.find(
      (e) =>
        e.type === 'membership.removed' && (e.data as { removed_owner?: string }).removed_owner === 'memberB',
    );
  ...
  it('promotes an heir when the departing identity was the SOLE owner (preserves ≥1 owner)', async () => {
    ...
    const res = await server.inject({
      method: 'DELETE',
      url: '/identities/ownerA/memberships',
      headers: SVC,
    });
    expect(res.statusCode).toBe(200);
    expect(res.json().groups_deleted).toHaveLength(0);
...
describe('principal teardown — idempotency + safety', () => {
  it('identity delete on an absent identity is a 200 no-op (not a 404)', async () => {
    const res = await server.inject({
      method: 'DELETE',
      url: '/auth/admin/identity/user_ghost',
      headers: SVC,
    });
    expect(res.statusCode).toBe(200);
  ...
  it('billing-customer delete drops the record even when Stripe is NOT configured', async () => {
    ...
    const res = await server.inject({
      ...
      url: '/billing/customer',
      headers: SVC,
      payload: { subscriber: 'unconf_user' },
    });
    expect(res.statusCode).toBe(200);
    expect(res.json()).toMatchObject({
      deleted: true,
  ...
  it('membership teardown for an identity with no memberships is a clean 200 no-op', async () => {
    ...
    const res = await server.inject({
      method: 'DELETE',
      url: '/identities/nobody/memberships',
      headers: SVC,
    });
    expect(res.statusCode).toBe(200);
    expect(res.json()).toMatchObject({
...
describe('principal teardown — service-gating (NOT end-user reachable)', () => {
  it('every teardown op is refused (401) without a valid service token', async () => {
    ...
    expect(
      (await server.inject({ method: 'DELETE', url: `/auth/admin/identity/${user.id}`, headers: wrongToken }))
        .statusCode,
    ).toBe(401);
    expect(
      (
...
describe('admin identity enumeration — GET /auth/admin/identities (the "list all accounts" seam)', () => {
  it('lists EVERY identity for the app (full email, derived provider, created_at) with a service token', async () => {
    const google = await authStore.createUser(APP_ID, {
      email: 'ExecG@Demo.test',
      email_verified: true,
      provider: 'google',
      provider_user_id: 'g-1',
    });
    const pw = await authStore.createUser(APP_ID, {
      email: 'pw@demo.test',
      email_verified: true,
      password_hash: await hashPassword('password123'),
    });
    ...
    const byId = new Map<
      string,
      { user_id: string; email: string | null; provider: string | null; created_at: string | null }
    >(
      res
        .json()
        .identities.map(
    ...
    expect(byId.get(google.id)).toMatchObject({
      user_id: google.id,
      email: 'execg@demo.test',
      provider: 'google',
    });
