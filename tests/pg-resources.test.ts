...
const agentTask = (id: string, over: Partial<AgentTask> = {}): AgentTask => ({
  ...
  model: 'claude-x',
  artifact: { ok: true },
  implementation: 'model-anthropic',
  ...over,
});

describe.skipIf(!HAS_PG)(
  'P26 Postgres resource backend — jsonb round-trip, O4, claim-legacy, P27, backfill',
  () => {
    let pool: Pool;
    beforeAll(() => {
    ...
    it('owner-scoped list excludes owner-less; claim-legacy stamps them (idempotent)', async () => {
      ...
      expect(
        (await store.listResources({ type: 'AgentTask', app_id: 'app_r', owner: 'A' })).map((r) => r.id),
      ).toEqual(['at_owned']);
    ...
    it('backfill (filesystem → Postgres) preserves resource ids + shapes', async () => {
      ...
      try {
        ...
        expect((await pg.list({ type: 'AgentTask', app_id: 'app_bf' })).map((r) => r.id).sort()).toEqual([
          'at_bf1',
          'at_bf2',
        ]);
      } finally {
