...
describe.skipIf(!HAS_PG)('P26 Postgres search backend — GIN index, O4 scope, escaping, backfill', () => {
  ...
  it('a doc indexed WITHOUT ACL fields defaults to owner-only (visibility=private, group_id NULL, empty grants)', async () => {
    ...
    expect(
      (await search.search(APP, { owner: 'B', q: 'alpha', scope: { groupId: 'house', canReadAll: true } }))
        .total,
    ).toBe(0);
  });
  ...
  it('access-aware predicate in SQL: group⇒read-all same-group, shared⇒grantees only, private stays private, owner always sees own', async () => {
    ...
    expect(
      (
        await search.search(APP, { owner: 'M', q: 'alpha', scope: { groupId: 'house', canReadAll: true } })
      ).hits
        .map((h) => h.id)
        .sort(),
    ).toEqual(['grp']);
    ...
    expect(
      (
        await search.search(APP, { owner: 'B', q: 'alpha', scope: { groupId: 'house', canReadAll: false } })
      ).hits.map((h) => h.id),
    ).toEqual(['shr']);
    // W matches via sharedWriters (union).
    expect(
      (
        await search.search(APP, { owner: 'W', q: 'alpha', scope: { groupId: 'house', canReadAll: false } })
      ).hits.map((h) => h.id),
    ).toEqual(['shr']);
    // Cross-group read-all member sees NOTHING (the group predicate is same-group only).
    expect(
      (await search.search(APP, { owner: 'C', q: 'alpha', scope: { groupId: 'other', canReadAll: true } }))
        .total,
    ).toBe(0);
    // A non-granted, non-read-all same-group member sees nothing of A's.
    expect(
      (await search.search(APP, { owner: 'Z', q: 'alpha', scope: { groupId: 'house', canReadAll: false } }))
        .total,
    ).toBe(0);
  });
  ...
  it('ACL narrows BEFORE limit/paging in SQL — total is the ACL-scoped count, private docs never page in', async () => {
    ...
    for (let off = 0; off < 5; off += 2)
      for (const h of (await search.search(APP, { owner: 'M', q: 'alpha', limit: 2, offset: off, scope }))
        .hits)
        seen.add(h.id);
  ...
  it('never leaks raw HTML from document content into the snippet; highlights matches with <mark>', async () => {
    ...
    await search.index(APP, {
      ...
      id: 'x',
      title: 'Report',
      body: 'Revenue & <script>alert(1)</script> for the alpha quarter',
    });
  ...
  it('backfill (filesystem → Postgres) relocates the index with (owner,type,id) keys preserved', async () => {
    ...
    try {
      ...
      expect(
        (
          await pg.search(APP2, { owner: 'M', q: 'roster', scope: { groupId: 'house', canReadAll: true } })
        ).hits.map((h) => h.id),
      ).toEqual(['shared']);
    } finally {
