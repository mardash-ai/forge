...
const HAS_S3 =
  process.env.FORGE_BLOBS_BACKEND === 's3' &&
  Boolean(process.env.FORGE_DB_URL) &&
  Boolean(process.env.FORGE_S3_ENDPOINT);
...
describe.skipIf(!HAS_S3)(
  'P26 S3+Postgres blob backend — object store bytes, PG metadata, rollback, backfill',
  () => {
    ...
    const commit = async (owner: string, data: Buffer, blobId: string): Promise<BlobMetadata> => {
      ...
      const res = await backend.commit(APP, tmp, meta, {
        maxBytes: 1e9,
        quotaBytes: 1e9,
        quotaObjects: 1000,
    ...
    it('serves a byte range from the object store', async () => {
      ...
      const partial = await streamToBuffer(
        await backend.openRange(APP, m.blob_id, { start: PNG_SIG.length, end: PNG_SIG.length + 3 }),
      );
    ...
    it('backfill (filesystem → object store + Postgres) preserves the blob_id, bytes, and metadata', async () => {
      ...
      try {
        ...
        const fsRes = await blobStore.commit(
          APP2,
          tmp,
          {
            blob_id: blobId,
