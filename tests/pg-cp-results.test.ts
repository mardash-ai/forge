...
import {
  PgCpResultsBackend,
  ensureCpResultsSchema,
} from "../src/storage/backends/cp-results/pg";
...
const HAS_PG =
  process.env.FORGE_CP_RESULTS_BACKEND === "postgres" &&
...
describe.skipIf(!HAS_PG)(
  ...
  () => {
    ...
    it("updateRun — returns null for unknown run_id", async () => {
      const r = await backend.updateRun("run_does_not_exist", {
        status: "failed",
      });
      expect(r).toBeNull();
    });
    ...
    it("insertClaim — stores structured cassette; listClaims ordered", async () => {
      ...
      expect(claims[0]!.cassette).toMatchObject({
        prompt: "Summarize the document",
      });
      expect((claims[0]!.cassette as { turns: unknown[] }).turns).toHaveLength(
        2,
      );
    ...
    it("FK cascade — deleting a run removes all child rows", async () => {
      await pool.query("DELETE FROM forge_cp_eval_runs WHERE run_id=$1", [
        RUN_ID,
      ]);
      const wfs = await backend.listWorkflows(RUN_ID);
