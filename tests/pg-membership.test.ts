...
describe.skipIf(!HAS_PG)(
  'P26 Postgres membership backend — jsonb document, serialized mutate, backfill',
  () => {
    ...
    it('read is empty until written; mutate persists roles + a provisioned group in ONE row', async () => {
      ...
      expect(state.members[memberKey('grp_1', 'A')]).toMatchObject({
        owner: 'A',
        role: 'owner',
        status: 'active',
      });
      ...
      const n = await pool.query<{ n: string }>(
        'SELECT count(*)::text AS n FROM forge_membership WHERE app_id=$1',
        [APP],
      );
        ...
        Array.from({ length: 8 }, () =>
          b.mutate(APP2, (s) =>
            provisionGroup(s, {
              owner: 'Z',
              now: nowIso(),
              newGroupId: `grp_${Math.random().toString(36).slice(2)}`,
    ...
    it('backfill (filesystem → Postgres) copies the whole membership document', async () => {
      ...
      try {
        ...
        await fs.mutate(APP3, (s) =>
          provisionGroup(s, { owner: 'A', now: nowIso(), newGroupId: 'grp_bf', dedupeOwnerSingleton: true }),
        );
        ...
        expect(await backfillMembership(fs, pg, [APP3])).toEqual([
          { app: APP3, groups: 1, members: 1, invitations: 0, roles: 2 },
        ]);
