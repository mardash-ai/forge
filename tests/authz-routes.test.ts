...
describe('C29 — policy CRUD', () => {
  it('create → list → get → delete a policy', async () => {
    ...
    expect((await get('/policies?owner=A')).json().policies.map((p: { id: string }) => p.id)).toContain(
      policy.id,
    );
  ...
  it('validates: missing effect → 422; unknown app → 404', async () => {
    ...
    await bare.ready();
    expect(
      (await bare.inject({ method: 'POST', url: '/policies', payload: { effect: 'allow', app: 'nope' } }))
        .statusCode,
    ).toBe(404);
    await bare.close();
  });
...
describe('C29 — policy REMOVAL (owner-scoped, idempotent, event-emitting)', () => {
  it('remove → authorize no longer applies the rule', async () => {
    ...
    const id = (await post('/policies', { owner: 'A', effect: 'allow', match: { type: ['read'] } })).json()
      .policy.id;
  ...
  it('scope isolation: a caller cannot remove another owner’s rule, nor an app-wide rule, via its owner scope', async () => {
    const aRule = (await post('/policies', { owner: 'A', effect: 'allow', match: { type: ['read'] } })).json()
      .policy.id;
    const appWide = (await post('/policies', { effect: 'allow', match: { type: ['read'] } })).json().policy
      .id; // no owner
    ...
    expect(
      (await post('/authorize', { owner: 'A', action: { type: 'read', reversibility: 'reversible' } })).json()
        .decision,
    ).toBe('allow'); // still enforcing
  ...
  it('emits policy.set on create and policy.removed on a real removal (owner-scoped, C3 audit)', async () => {
    ...
    expect(events.some((e) => e.type === 'policy.removed' && (e.data as { id?: string }).id === id)).toBe(
      true,
    );
...
describe('C29 — POST /authorize (deterministic + C3 audit)', () => {
  it('a matching allow policy → allow, and the decision is recorded to C3', async () => {
    ...
    const r = await post('/authorize', {
      owner: 'A',
      action: { tool: 'get_note', type: 'read', reversibility: 'reversible' },
    });
    ...
    expect(
      events.some(
        (e) => e.type === 'authz.decision' && (e.data as { decision?: string }).decision === 'allow',
      ),
    ).toBe(true);
  });
  ...
  it('high-risk action always stages (needs-approval), even with a matching allow policy (safety floor)', async () => {
    ...
    const r = await post('/authorize', {
      owner: 'A',
      action: { tool: 'send_email', type: 'send', channel: 'email', contact: 'x@y.com' },
    });
    const d = r.json();
  ...
  it('no policy → the conservative default; owner-scoping isolates A from B', async () => {
    ...
    expect(
      (await post('/authorize', { owner: 'B', action: { type: 'read', reversibility: 'reversible' } })).json()
        .decision,
    ).toBe('needs-approval');
  });
...
describe('C29 — progressive autonomy (approvals via C3)', () => {
  it('records approvals and surfaces "approved N times" with a policy suggestion at the threshold', async () => {
    for (let i = 0; i < 3; i++)
      await post('/authz/approvals', { owner: 'A', action_class: 'send_email:email' });
