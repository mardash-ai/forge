...
describe.skipIf(!HAS_PG)(
  'P26 Postgres notification backend — upsert, sentinel, O4, concurrency, backfill',
  () => {
    ...
    it('upsert is one ON CONFLICT (no whole-map rewrite): re-derive updates in place, preserves dismissed + created_at', async () => {
      ...
      const rows = await pool.query(
        'SELECT count(*)::int AS n FROM forge_notifications WHERE app_id=$1 AND owner=$2 AND key=$3',
        [APP, 'A', 'k'],
      );
