...
const mem = (over: Partial<ResolvedMembership> = {}): ResolvedMembership => ({
  group_id: 'g',
  role: 'member',
  permissions: [],
  is_member: true,
...
describe('C31 — pure authorize: BACK-COMPAT (no membership ⇒ identical, no new fields)', () => {
  it('a legacy call is byte-identical to pre-C31 (no role/permissions/is_member/group_id)', () => {
    ...
    expect(d).toEqual({
      decision: 'allow',
      rule: 'allow_reads',
      reason: 'governed by policy allow_reads',
      high_risk: false,
      action_class: 'get_note',
    });
  ...
  it('the request role is still honored when NO membership is resolved (legacy)', () => {
    ...
    expect(authorize(actor({ role: 'member' }), read(), policies, { now: NOW }).decision).toBe(
      'needs-approval',
    );
  });
});
...
describe('C31 — pure authorize: resolved role + floors + permission gating', () => {
  it('matches on the RESOLVED role, not the request (role override)', () => {
    ...
    const d = authorize(actor(), read(), policies, {
      now: NOW,
      membership: mem({ role: 'owner', is_member: true, personal: true }),
    });
  ...
  it('NOT-A-MEMBER floor: a non-personal group you do not belong to → deny not-a-member', () => {
    ...
    const d = authorize(actor(), read(), allowAll, {
      now: NOW,
      membership: mem({ is_member: false, role: undefined, personal: false }),
    });
  ...
  it('PRIVATE-LEAK floor: private row owned by someone else → deny private-resource', () => {
    ...
    const d = authorize(
      actor({ owner: 'B' }),
      read({ visibility: 'private', resource_owner: 'A' }),
      allowAll,
      { now: NOW, membership: mem({ group_id: 'g', is_member: true }) },
    );
    expect(d).toMatchObject({ decision: 'deny', rule: 'private-resource' });
    // the owner of a private row is NOT leaked-against
    expect(
      authorize(actor({ owner: 'A' }), read({ visibility: 'private', resource_owner: 'A' }), allowAll, {
        now: NOW,
        membership: mem({ is_member: true }),
      }).decision,
    ).toBe('allow');
  });
  ...
  it('PRIVATE-LEAK floor: shared row denies a caller who is neither owner nor in shared_with', () => {
    ...
    const denied = authorize(
      actor({ owner: 'C' }),
      read({ visibility: 'shared', resource_owner: 'A', shared_with: ['B'] }),
      allowAll,
      { now: NOW, membership: mem({ is_member: true }) },
    );
  ...
  it('group-visible rows never trigger the private-leak floor (membership already gated)', () => {
    const allowAll = [rule({ id: 'yolo', effect: 'allow', priority: 100, match: {} })];
    expect(
      authorize(actor({ owner: 'B' }), read({ visibility: 'group', resource_owner: 'A' }), allowAll, {
        now: NOW,
        membership: mem({ is_member: true }),
      }).decision,
    ).toBe('allow');
  ...
  it('PERMISSION gating: a rule with `permission` matches only when the resolved set holds it', () => {
    const policies = [
      rule({ id: 'perm', effect: 'allow', match: { type: ['read'], permission: ['notes.read'] } }),
    ];
    expect(
      authorize(actor(), read(), policies, { now: NOW, membership: mem({ permissions: ['notes.read'] }) })
        .decision,
    ).toBe('allow');
    expect(
      authorize(actor(), read(), policies, { now: NOW, membership: mem({ permissions: [] }) }).decision,
    ).toBe('needs-approval');
  });
  ...
  it('the safety floor still fires under C31 (a resolved role cannot downgrade high-risk)', () => {
    ...
    const d = authorize(
      actor(),
      { tool: 'send_email', type: 'send', channel: 'email', contact: 'x@y.com' },
      allowAll,
      { now: NOW, membership: mem({ role: 'owner' }) },
    );
...
describe('C31 — /authorize route: resolution, legacy path, and floors end-to-end', () => {
  ...
  it('PRIVATE-LEAK deny end-to-end: a caller acting on another owner’s private row is denied', async () => {
    ...
    const d = await authorizeCall({
      owner: 'B',
      action: read({ visibility: 'private', resource_owner: 'A' }),
    });
    expect(d).toMatchObject({ decision: 'deny', rule: 'private-resource' });
  });
