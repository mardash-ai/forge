...
describe('C11 owner-scoping over HTTP (C3 + C4)', () => {
  ...
  it('C4: a notification list scoped to owner A never returns owner B’s — same key stays distinct', async () => {
    ...
    const aList = (await get('/notifications?owner=A')).notifications as Array<{
      title: string;
      owner: string;
    }>;
  ...
  it('claim-legacy migrates owner-less C3 + C4 records to a seeded owner, and is idempotent', async () => {
    ...
    expect(((await get('/app-events?owner=owner-1')).events as unknown[]).map((e: any) => e.type)).toEqual([
      'owned-evt',
    ]);
    ...
    expect(
      ((await get('/app-events?owner=owner-1')).events as unknown[]).map((e: any) => e.type).sort(),
    ).toEqual(['legacy-evt', 'owned-evt']);
    expect(
      ((await get('/notifications?owner=owner-1')).notifications as unknown[]).map((n: any) => n.key),
    ).toEqual(['legacy-note']);

    // Idempotent — a second run claims nothing.
    expect((await post('/owner/claim-legacy', { owner: 'owner-1' })).json().claimed).toMatchObject({
      app_events: 0,
      notifications: 0,
    });
  });
