...
async function enable2fa(cookie: string) {
  ...
  const confirm = await server.inject({
    ...
    url: '/auth/2fa/enable',
    headers: { ...JSON_HDR, cookie },
    payload: JSON.stringify({ code: lastCode() }),
  });
...
describe('opt-in safety — a non-2FA login is byte-for-byte unchanged', () => {
  it('logs in with an immediate session, sends NO code, and reports twofa_enabled:false', async () => {
    ...
    const me = await server.inject({
      method: 'GET',
      url: '/auth/session',
      headers: { ...HDR, cookie: `forge_session=${cookieValue(login, 'forge_session')}` },
    });
  ...
  it('a JSON login for a non-2FA user is NOT diverted to a challenge', async () => {
    ...
    const login = await server.inject({
      ...
      url: '/auth/login',
      headers: JSON_HDR,
      payload: JSON.stringify({ email: 'jane@example.com', password: 'correct horse battery' }),
    });
...
describe('change password (authenticated)', () => {
  it('changes the password, signs out other devices, keeps THIS device, and enforces the policy', async () => {
    ...
    const wrong = await server.inject({
      ...
      url: '/auth/password',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ current_password: 'nope', new_password: 'brand-new-pass-2' }),
    });
    ...
    const weak = await server.inject({
      ...
      url: '/auth/password',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ current_password: 'original-pass-1', new_password: 'short' }),
    });
    ...
    const ok = await server.inject({
      ...
      url: '/auth/password',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ current_password: 'original-pass-1', new_password: 'brand-new-pass-2' }),
    });
    ...
    expect(
      (
        await server.inject({
          method: 'GET',
          url: '/auth/session',
          headers: { ...HDR, cookie: `forge_session=${freshSession}` },
        })
      ).statusCode,
    ).toBe(200);
    ...
    expect(
      (
        await server.inject({
          method: 'POST',
          url: '/auth/refresh',
          headers: { ...HDR, cookie: `forge_refresh=${device2Refresh}` },
        })
      ).statusCode,
    ).toBe(401);
    ...
    expect(
      (
        await server.inject({
          method: 'POST',
          url: '/auth/refresh',
          headers: { ...HDR, cookie: `forge_refresh=${otherRefresh}` },
        })
      ).statusCode,
    ).toBe(401);
  ...
  it('rejects an unauthenticated change with 401', async () => {
    ...
    const res = await server.inject({
      ...
      url: '/auth/password',
      headers: JSON_HDR,
      payload: JSON.stringify({ current_password: 'x', new_password: 'yyyyyyyy' }),
    });
  ...
  it('a Google-only account (no password) is a clean 409 no_password', async () => {
    ...
    const res = await server.inject({
      ...
      url: '/auth/password',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ current_password: '', new_password: 'brand-new-pass-2' }),
    });
...
describe('email 2FA — enroll, challenge, verify, resend, disable', () => {
  ...
  it('a 2FA-enabled login returns a challenge (no session) and completes only via /auth/2fa/verify', async () => {
    ...
    const bad = await server.inject({
      ...
      url: '/auth/2fa/verify',
      headers: JSON_HDR,
      payload: JSON.stringify({ challenge, code: '000000' }),
    });
    ...
    const done = await server.inject({
      ...
      url: '/auth/2fa/verify',
      headers: JSON_HDR,
      payload: JSON.stringify({ challenge, code: lastCode() }),
    });
    ...
    expect(
      (
        await server.inject({
          method: 'GET',
          url: '/auth/session',
          headers: { ...HDR, cookie: `forge_session=${sess}` },
        })
      ).statusCode,
    ).toBe(200);
  });
  ...
  it('the hosted HTML login renders an enter-code page and the form completes login', async () => {
    ...
    const login = await server.inject({
      ...
      url: '/auth/login',
      headers: FORM,
      payload: form({ email: 'jane@example.com', password: 'correct horse battery', next: '/dashboard' }),
    });
    expect(login.statusCode).toBe(200);
    ...
    const done = await server.inject({
      ...
      url: '/auth/2fa/verify',
      headers: FORM,
      payload: form({ challenge, code: lastCode(), next: '/dashboard' }),
    });
  ...
  it('resend issues a NEW code (old one invalid) that completes login', async () => {
    ...
    if (firstCode !== secondCode) {
      const stale = await server.inject({
        ...
        url: '/auth/2fa/verify',
        headers: JSON_HDR,
        payload: JSON.stringify({ challenge, code: firstCode }),
      });
    ...
    const done = await server.inject({
      ...
      url: '/auth/2fa/verify',
      headers: JSON_HDR,
      payload: JSON.stringify({ challenge, code: secondCode }),
    });
    expect(done.statusCode).toBe(200);
    expect(cookieValue(done, 'forge_session')).toBeTruthy();
  });
  ...
  it('disable via current password turns 2FA off — login returns to an immediate session', async () => {
    ...
    const wrong = await server.inject({
      ...
      url: '/auth/2fa/disable',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ password: 'WRONG' }),
    });
    expect(wrong.statusCode).toBe(403);
    // Turn it back off with the emailed code path.
    const startCode = await server.inject({
      method: 'POST',
      url: '/auth/2fa/disable',
      headers: { ...JSON_HDR, cookie },
      payload: '{}',
    ...
    const viaCode = await server.inject({
      method: 'POST',
      url: '/auth/2fa/disable',
      headers: { ...JSON_HDR, cookie },
      payload: JSON.stringify({ code: lastCode() }),
...
describe('Google OAuth honors 2FA', () => {
  it('a 2FA-enabled OAuth callback returns the challenge page, not a session', async () => {
    ...
    const done = await server.inject({
      ...
      url: '/auth/2fa/verify',
      headers: FORM,
      payload: form({ challenge, code: lastCode() }),
    });
    expect(done.statusCode).toBe(303);
    expect(cookieValue(done, 'forge_session')).toBeTruthy();
  });
