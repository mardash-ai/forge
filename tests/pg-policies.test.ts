...
describe.skipIf(!HAS_PG)(
  'P26 Postgres policy backend — jsonb + O4, owner/app-wide list, upsert, backfill',
  () => {
    ...
    it('round-trips a policy through jsonb, with O4 columns projected', async () => {
      const p = rule('pol_1', {
        ...
        effect: 'deny',
        priority: 7,
        match: { tool: ['send_email'], max_amount: 50 },
        reason: 'no',
      });
    ...
    it('put is a single upsert — one row after re-put', async () => {
      ...
      const n = await pool.query<{ n: string }>(
        "SELECT count(*)::text AS n FROM forge_policies WHERE app_id=$1 AND id='up'",
        [APP],
      );
