...
export class PgMcpBackend implements McpBackend, MigratableMcpBackend {
  ...
  async getTool(appId: string, name: string): Promise<ToolRegistration | null> {
    const r = await this.pool.query<DataRow<ToolRegistration>>(
      'SELECT data FROM forge_mcp_tools WHERE app_id=$1 AND name=$2',
      [appId, name],
    );
  ...
  async listTools(appId: string): Promise<ToolRegistration[]> {
    const r = await this.pool.query<DataRow<ToolRegistration>>(
      'SELECT data FROM forge_mcp_tools WHERE app_id=$1 ORDER BY name ASC',
      [appId],
    );
  ...
  async latestInstructions(appId: string): Promise<InstructionBlock | null> {
    const r = await this.pool.query<DataRow<InstructionBlock>>(
      'SELECT data FROM forge_mcp_instructions WHERE app_id=$1 ORDER BY version DESC LIMIT 1',
      [appId],
    );
    return r.rows[0]?.data ?? null;
  }
  async getInstructions(appId: string, version: number): Promise<InstructionBlock | null> {
    const r = await this.pool.query<DataRow<InstructionBlock>>(
      'SELECT data FROM forge_mcp_instructions WHERE app_id=$1 AND version=$2',
      [appId, version],
    );
    return r.rows[0]?.data ?? null;
  }
  async listInstructions(appId: string): Promise<InstructionBlock[]> {
    const r = await this.pool.query<DataRow<InstructionBlock>>(
      'SELECT data FROM forge_mcp_instructions WHERE app_id=$1 ORDER BY version ASC',
      [appId],
    );
    return r.rows.map((row) => row.data);
  }
  ...
  async getClient(appId: string, clientId: string): Promise<OAuthClient | null> {
    const r = await this.pool.query<DataRow<OAuthClient>>(
      'SELECT data FROM forge_mcp_clients WHERE app_id=$1 AND client_id=$2',
      [appId, clientId],
    );
    return r.rows[0]?.data ?? null;
  }
  ...
  async getConsent(appId: string, clientId: string, owner: string): Promise<Consent | null> {
    const r = await this.pool.query<DataRow<Consent>>(
      'SELECT data FROM forge_mcp_consents WHERE app_id=$1 AND client_id=$2 AND owner=$3',
      [appId, clientId, owner],
    );
    return r.rows[0]?.data ?? null;
  }
  async listConsents(appId: string, owner: string): Promise<Consent[]> {
    const r = await this.pool.query<DataRow<Consent>>(
      'SELECT data FROM forge_mcp_consents WHERE app_id=$1 AND owner=$2',
      [appId, owner],
    );
    return r.rows.map((row) => row.data);
  }
  async revokeConsent(appId: string, clientId: string, owner: string): Promise<boolean> {
    ...
    try {
      ...
      const r = await client.query(
        'DELETE FROM forge_mcp_consents WHERE app_id=$1 AND client_id=$2 AND owner=$3',
        [appId, clientId, owner],
      );
      // Cut the connector off now: drop this user's live tokens for the client.
      await client.query('DELETE FROM forge_mcp_grants WHERE app_id=$1 AND client_id=$2 AND owner=$3', [
        appId,
        clientId,
        owner,
      ]);
  ...
  async putGrant(appId: string, grant: OAuthGrant): Promise<OAuthGrant> {
    await this.pool.query(
      ...
      [
        ...
        grant.group_id ?? null,
        grant.visibility ?? 'private',
      ],
    );
    return grant;
  }
  async getGrant(appId: string, kind: GrantKind, tokenHash: string): Promise<OAuthGrant | null> {
    const r = await this.pool.query<DataRow<OAuthGrant>>(
      'SELECT data FROM forge_mcp_grants WHERE app_id=$1 AND kind=$2 AND token_hash=$3',
      [appId, kind, tokenHash],
    );
    return r.rows[0]?.data ?? null;
  }
  async consumeGrant(appId: string, kind: GrantKind, tokenHash: string): Promise<OAuthGrant | null> {
    const r = await this.pool.query<DataRow<OAuthGrant>>(
      'DELETE FROM forge_mcp_grants WHERE app_id=$1 AND kind=$2 AND token_hash=$3 RETURNING data',
      [appId, kind, tokenHash],
    );
    return r.rows[0]?.data ?? null;
  }
  async revokeGrant(appId: string, kind: GrantKind, tokenHash: string): Promise<boolean> {
    const r = await this.pool.query(
      'DELETE FROM forge_mcp_grants WHERE app_id=$1 AND kind=$2 AND token_hash=$3',
      [appId, kind, tokenHash],
    );
    return (r.rowCount ?? 0) > 0;
  }
  async revokeUserGrants(appId: string, owner: string): Promise<number> {
    const r = await this.pool.query('DELETE FROM forge_mcp_grants WHERE app_id=$1 AND owner=$2', [
      appId,
      owner,
    ]);
    return r.rowCount ?? 0;
  ...
  async exportApp(appId: string): Promise<McpExport> {
    const [tools, instructions, clients, consents, grants] = await Promise.all([
      ...
      this.pool.query<DataRow<InstructionBlock>>(
        'SELECT data FROM forge_mcp_instructions WHERE app_id=$1 ORDER BY version ASC',
        [appId],
      ),
  ...
  async importApp(appId: string, data: McpExport): Promise<void> {
    ...
    try {
      ...
      for (const t of data.tools) {
        await client.query(
          'INSERT INTO forge_mcp_tools (app_id, name, data, updated_at) VALUES ($1,$2,$3::jsonb,$4)',
          [appId, t.name, JSON.stringify(t), t.updated_at],
        );
      }
      for (const b of data.instructions) {
        await client.query(
          'INSERT INTO forge_mcp_instructions (app_id, version, data, created_at) VALUES ($1,$2,$3::jsonb,$4)',
          [appId, b.version, JSON.stringify(b), b.created_at],
        );
      }
      for (const c of data.clients) {
        await client.query(
          'INSERT INTO forge_mcp_clients (app_id, client_id, data, created_at) VALUES ($1,$2,$3::jsonb,$4)',
          [appId, c.client_id, JSON.stringify(c), c.created_at],
        );
      }
      for (const c of data.consents) {
        await client.query(
          'INSERT INTO forge_mcp_consents (app_id, client_id, owner, data, group_id, visibility, updated_at) VALUES ($1,$2,$3,$4::jsonb,$5,$6,$7)',
          [
            appId,
            c.client_id,
            c.owner,
      ...
      for (const g of data.grants) {
        await client.query(
          'INSERT INTO forge_mcp_grants (app_id, kind, token_hash, owner, client_id, expires_at, data, group_id, visibility) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9)',
          [
            appId,
            g.kind,
            g.token_hash,
  ...
  async __truncateAllForTests(): Promise<void> {
    await this.pool.query(
      'TRUNCATE forge_mcp_tools, forge_mcp_instructions, forge_mcp_clients, forge_mcp_consents, forge_mcp_grants',
    );
  }
}
