...
//
//   POST /notifications           { app?, key, title, body?, data?, subject?, owner? }  -> { notification }
...
//
// `owner` (C11) is the opaque per-user id (C10's session `userId`). Scoping is by (app, owner, key):
// two users may hold the same `key` as DISTINCT notifications, dismiss/clear act on the caller's own,
// and a list scoped to an owner returns ONLY that owner's — user A never sees user B. Omitting `owner`
// is app-scoped (all owners), so a C10-less app and pre-C11 notifications keep working unchanged.
export function registerNotificationRoutes(
  ...
  app.post('/notifications', async (req, reply) => {
    const b = (req.body ?? {}) as { app?: string; key?: string; title?: string; body?: string; data?: Record<string, unknown>; subject?: string; owner?: string };
    ...
    const notification = await store.upsertNotification(app_id, { key: b.key, title: b.title, body: b.body, data: b.data, subject: b.subject, owner: b.owner });
