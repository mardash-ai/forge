...
export function registerBlobRoutes(
  ...
  const unknownApp = blobError(
    'not_found',
    'unknown app (pass `app` or set FORGE_APP_NAME).',
    'change-input',
  );
  ...
  const collect = async (
    ...
    const parts = req.parts({
      limits: {
        fileSize: Math.min(config.maxBytes, HARD_FILE_CEILING),
        files: 1,
        fields: 25,
        fieldSize: 1_000_000,
      },
  ...
  app.post('/blobs', async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      return reply
        ...
        .send(
          blobError(
            ...
            'POST /blobs expects multipart/form-data with a `file` field.',
            'change-input',
          ),
        );
    }
    ...
    if (!up.found) {
      ...
      return reply
        .status(400)
        .send(blobError('missing_file', 'the multipart body must include a `file` part.', 'change-input'));
    }
    ...
    if (up.truncated) {
      ...
      return reply
        .status(413)
        .send(blobError('file_too_large', `file exceeds the ${config.maxBytes}-byte limit.`, 'change-input'));
    }
    ...
    if (!owner) {
      ...
      return reply
        .status(422)
        .send(blobError('invalid_input', 'an upload requires a non-empty `owner`.', 'change-input'));
    }
    ...
    if (!declared) {
      ...
      return reply
        ...
        .send(
          blobError(
            'unsupported_media_type',
            'declare a `content_type` (or send a typed file part).',
            'change-input',
          ),
        );
    }
    if (!config.allowedTypes.has(declared)) {
      ...
      return reply
        .status(415)
        .send(
          blobError('unsupported_media_type', `content type "${declared}" is not allowed.`, 'change-input'),
        );
    }
    ...
    if (!sniffMatches(declared, up.head)) {
      ...
      return reply
        ...
        .send(
          blobError(
            'content_mismatch',
            `file content does not match the declared type "${declared}".`,
            'change-input',
          ),
        );
    }
    ...
    if (up.fields.attrs !== undefined && up.fields.attrs !== '') {
      ...
      try {
        ...
        return reply
          .status(422)
          .send(blobError('invalid_input', '`attrs` must be a JSON object.', 'change-input'));
      }
    }
        ...
        return reply
          ...
          .send(
            blobError(
              ...
              `owner object quota (${config.quotaObjects}) reached.`,
              'change-input',
            ),
          );
      }
      return reply
        ...
        .send(
          blobError(
            ...
            `owner storage quota (${config.quotaBytes} bytes) would be exceeded.`,
            'change-input',
          ),
        );
    }
  ...
  app.get('/blobs/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (range === 'unsatisfiable') {
      return reply
        ...
        .send(blobError('range_not_satisfiable', 'requested range is out of bounds.', 'change-input'));
    }
  ...
  app.get('/blobs', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.status(200).send({
      ...
      usage: {
        ...
        quota_objects: config.quotaObjects,
      },
    });
  });
}
...
function mapStreamError(reply: FastifyReply, err: Error): FastifyReply {
  ...
  if (code === 'ENOSPC') {
    return reply
      .status(507)
      .send(blobError('storage_full', 'insufficient storage to persist the blob.', 'backoff'));
  }
  if (code === 'EIO' || code === 'EACCES' || code === 'EROFS') {
    return reply
      .status(503)
      .send(blobError('storage_unavailable', 'blob storage is temporarily unavailable.', 'backoff'));
  }
  // Default: the client went away mid-upload — the write was abandoned, nothing was stored.
  return reply
    .status(400)
    .send(blobError('upload_aborted', 'the upload did not complete; nothing was stored.', 'change-input'));
}
