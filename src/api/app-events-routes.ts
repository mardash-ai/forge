...
//
//   POST /app-events            { app?, type, subject?, data? }  -> { event }
//   GET  /app-events            ?app=&subject=&limit=            -> { events }   (newest-first)
//   GET  /app-events/latest     ?app=                            -> { latest }   (subject -> ISO)
//
...
export function registerAppEventRoutes(
  ...
  app.post('/app-events', async (req, reply) => {
    const b = (req.body ?? {}) as { app?: string; type?: string; subject?: string; data?: Record<string, unknown> };
    ...
    const event = await store.appendAppEvent({ app_id, type: b.type, subject: b.subject, data: b.data });
    return reply.status(200).send({ event });
  });

  app.get('/app-events', async (req, reply) => {
    const q = req.query as { app?: string; subject?: string; limit?: string };
    const app_id = await resolveAppId(q.app);
    if (!app_id) return reply.status(404).send(unknownApp);
    const events = await store.listAppEvents({ app_id, subject: q.subject, limit: q.limit ? Number(q.limit) : undefined });
    return { events };
  });
  ...
  app.get('/app-events/latest', async (req, reply) => {
    ...
    const app_id = await resolveAppId(q.app);
    if (!app_id) return reply.status(404).send(unknownApp);
    return { latest: await store.latestAppEventTimes(app_id) };
  });
}
