...
export async function checkHealth(
  ...
  if (!probe.reachable) {
    return {
      ...
      target,
      expected,
      actual: `unreachable (${probe.error})`,
    };
  }
  if (isRedirect(probe.httpStatus)) {
    return {
      ...
      target,
      expected,
      actual: `HTTP ${probe.httpStatus} redirect to ${probe.redirectLocation ?? '?'}`,
      detail: 'health must be PUBLIC — it is behind an auth redirect',
    };
  }
  if (probe.httpStatus === 401 || probe.httpStatus === 403) {
    return {
      ...
      target,
      expected,
      actual: `HTTP ${probe.httpStatus}`,
      detail: 'health must be PUBLIC — it is auth-gated',
    };
  }
  ...
  if (!probe.conforms || !probe.health) {
    return {
      ...
      target,
      expected,
      actual: `HTTP 200 but body is not the C6 schema`,
      detail: probe.parseError,
    };
  }
...
export async function checkPageGate(
  ...
  if (!toLogin) {
    return {
      ...
      target,
      expected,
      actual: isRedirect(r.status)
        ? `HTTP ${r.status} → ${loc || '(no Location)'}`
        : `HTTP ${r.status} (no redirect to login)`,
    };
  }
...
export async function checkApiGate(baseUrl: string, apiPath: string, timeoutMs: number): Promise<Assertion> {
  ...
  if (!r.reachable) {
    return {
      name: `api-gate:${apiPath}`,
      title: `C10 API gate (${apiPath})`,
      status: 'fail',
      target,
      expected,
      actual: `unreachable (${r.error})`,
  ...
  return {
    ...
    target,
    expected,
    actual: isRedirect(r.status) ? `HTTP ${r.status} → ${r.location ?? '?'}` : `HTTP ${r.status}`,
    ...(detail ? { detail } : {}),
  };
}
...
export async function checkAuthConfig(
  baseUrl: string,
  expect: ExpectMethods | undefined,
  timeoutMs: number,
): Promise<Assertion> {
  ...
  const r = await httpProbe(`${baseUrl}/auth/config`, {
    redirect: 'manual',
    timeoutMs,
  ...
  try {
    ...
    const issue =
      e instanceof z.ZodError
        ? e.issues[0]
          ? `${e.issues[0].path.join('.') || '(root)'}: ${e.issues[0].message}`
          : 'shape mismatch'
        : 'not JSON';
    return {
      name: 'auth-config',
      ...
      target,
      expected,
      actual: `HTTP 200 but not the {methods,configured} shape`,
      detail: issue,
    };
  }
  ...
  if (missing.length) {
    return {
      ...
      target,
      expected,
      actual: `enabled: ${enabled.join(', ') || 'none'}`,
      detail: missing.join('; '),
    };
  }
  return {
    ...
    target,
    expected,
    actual: `HTTP 200, enabled: ${enabled.join(', ') || 'none'}`,
  };
}
...
export async function checkRefresh(baseUrl: string, timeoutMs: number): Promise<Assertion> {
  ...
  return {
    ...
    ...(r.status === 200 ? { detail: 'a cookie-less refresh must not mint a session' } : {}),
  };
}
