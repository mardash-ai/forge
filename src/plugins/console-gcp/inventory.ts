...
import type { Feature, InventoryProvider, ProviderContext, ProviderHealth } from '../../console/providers/types';
        ...
        ['cloud-run', async () => {
          ...
          return items.map((x) => {
            ...
            return R({
              ...
              location: region,
              labels: x['labels'] ?? {},
              state: x['terminalCondition']?.type === 'Ready' && x['terminalCondition']?.state === 'CONDITION_SUCCEEDED' ? 'Ready' : (x['terminalCondition']?.state ?? 'unknown'),
          ...
          return (p.items ?? []).map((x) =>
            R({
              ...
              attributes: {
                ...
                private_ip: (x['ipAddresses'] ?? []).find((i: any) => i.type === 'PRIVATE')?.ipAddress ?? null,
              },
        ...
        ['instances', async () => {
          ...
          return items.map((x) =>
            R({
              ...
              name: String(x['name']),
              scope: 'zonal',
              location: String(x['zone'] ?? '').split('/').pop(),
              state: x['status'],
              created_at: x['creationTimestamp'],
              attributes: {
                machine_type: String(x['machineType'] ?? '').split('/').pop() ?? null,
