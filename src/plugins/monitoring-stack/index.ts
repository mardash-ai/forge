...
interface ResolvedAppDb { network: string; host: string; port: number; database: string; user: string; appId: string }
  ...
  return {
    ...
    appDb: opts.appDb
      ? {
          ...
          host: opts.appDb.host,
          port: opts.appDb.port ?? 5432,
...
export function generateMonitoringCompose(opts: MonitoringStackOptions = {}): string {
  ...
  // appDb: grafana additionally joins the app stack's network so the read-only postgres
  // datasource (by-email user picker) can reach the DB by container name.
  const appDbNet = r.appDb ? `\n      - ${r.appDb.network}` : '';
  const grafanaNetworks = fronted
    ? `      - monitoring\n      - ${r.proxyNetwork}${appDbNet}`
    : `      - monitoring${appDbNet}`;
...
${traefikLabels}    configs:
      ...
      - source: dash-tool-drilldown
        target: /var/lib/grafana/dashboards/tool-drilldown.json${r.appDb ? `
      - source: dash-user-experience
        target: /var/lib/grafana/dashboards/user-experience.json` : ''}
    environment:
