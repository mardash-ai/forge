...
| **C19** | Search / full-text | **both** | `POST /index` · `POST /index/delete` · `POST /reindex` · `POST /search` — owner-scoped, BM25-ranked, `<mark>` snippets. Best-effort writes; soft `503` on internal search failure. |
