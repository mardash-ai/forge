...
| **C3** | App event log / timeline | **both** (routes on both servers) | `POST /app-events` · `GET /app-events?owner=&subject=&limit=` · `GET /app-events/latest` · `DELETE /app-events` (owner-scoped teardown). Best-effort write; owner-scoped read. **POST body**: `{ app?, type, subject?, owner?, caller?, data? }` — `caller` persists the emitter identity (e.g. OAuth client id, service name); absent ⇒ unattributed. **DELETE body**: `{ app?, owner }` — removes ALL events for that owner; never touches another tenant. **Forge-written events** (mcp.tool_call, authz.decision, policy.set/removed, connector.*, message.*) carry `data.trace_id` (W3C trace id from the active span) and a top-level `trace_id` field for cross-hop correlation; app-emitted events do not. |
