...
- **Gap 4 — `/auth/phone/*` production routing:** `registerSmsRoutes` (which mounts `/auth/phone/send-code` and `/auth/phone/verify-code`) is now registered on the data-plane server; the app's `next-config.ts` rewrites `/auth/:path*` to the data plane, so phone verification routes are now reachable in production.
...
> **Re-tag only — publish failed.** 1.56.1 was a re-tag of 1.56.0 whose publish-image.yml run did not produce a passing image; the next release supersedes it.
...
- **message-microsoft plugin** (`src/plugins/message-microsoft/`) — calls Microsoft Graph `POST /me/sendMail` as the authenticated user; registered as `email:microsoft` in the C25 send-message dispatch table alongside `email:google` (`requireScope: Mail.Send`, `implementation: message-microsoft`)
- **`account_label_claims` fallback** — connector `accountLabelFrom` tries `preferred_username` when `email` is absent from the OIDC id\_token; personal Microsoft accounts (MSA) are now labeled correctly
...
- **redirect-uri test isolation**: `tests/redirect-uri-fix.test.ts` now reads back registered clients from `getBackends().mcp` (the active backend — FS or Postgres, whichever the route wrote to) instead of a hardcoded `new FsMcpBackend()`. When the verify gate sets `TEST_DATABASE_URL` the vitest config activates the Postgres MCP backend, so the route wrote to PG but the test read from FS — a systematic backend mismatch that caused two integrity-check assertions to always fail under the gate.
...
- When `updated_at` has not advanced for more than 2 minutes the bar turns muted and surfaces
  the stall age (`stalled · updated N ago`), so a stuck run reads as stuck rather than quietly busy.
...
- **`GET /login` — the console's one unauthenticated page.** Server-rendered, branded, standalone
  (the SPA and its assets stay entirely behind the session). States the access policy — Google
  sign-in only, restricted to mardash.ai organization members — and offers the single way in.
  Unauthenticated browser navigation now 302s to `/login` in every auth mode; refused sign-ins
  land there with the reason ("not a member of the mardash.ai organization") instead of an error
  code on a page that immediately re-redirects; with no identity provider configured the page says
  sign-in is unavailable.

### Changed

- **Sign-out flow lands on `/login?signed_out=1`** (was: straight back into Google's authorize
  URL). The login page's button then carries `prompt=select_account`, preserving the account
  chooser. Sign-out now also writes an `auth.signout` audit row under the signed-in identity.
- **The identity + Log out control moved to the bottom of the nav rail** (the dorinda web app's
  placement) — the header keeps only the auth-mode label for open/dev deployments. The SPA
  returns to `/login` on any 401 (session expiry or revocation mid-use).
- The `[1.19.7]` changelog entry was rewritten — it contained a full pasted vitest log where the
  Fixed entry belonged (flagged by the acceptance instance).
...
- **`make test` (docker-compose run) is green locally again.** The three test files that import
  from `console/src/lib/` and the release-hygiene test in `infra.test.ts` failed inside the
  container because `compose.yaml` did not bind-mount `./console`, `./CHANGELOG.md`, or
  `./terraform`. CI runs `npm test` directly against the checkout so it never saw this; the
  mounts are now present. (This entry previously contained a full pasted vitest log by mistake —
  flagged by the acceptance instance, cleaned 2026-08-05.)
